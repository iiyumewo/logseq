...
-
- ## 审视 Git 规范
- 我们通过 Linux 内核开发工作流程中，围绕 Git 展开的几个重要基本原则来审视我们的 Git 规范，有哪些原则是通过 Lint 工具保障的，有哪些原则则是需要我们主动遵守的。Linux 规则只截取部分段落，完整部分可以在[这里](https://cloud.tencent.com/developer/news/681800)查看。
- ### 每次 commit 只能做一件事
...
- 如此依赖，任何对错误节点的追溯都可以通过二分的方式在 commit 节点中一点点寻找，有限的提交中 log2(n) 的时间复杂度可以保证在10来次的编译/测试中，一定能从海量 commit 中寻找到问题所在，而不用担心遇到不可正常工作的代码版本。
- ### 永远不要 rebase 公共分支
...
- 这部分是我们在提交规范中主要致力于达成的效果。通过上述章节的配置，我们已经可以在本地仓库中维持良好的 commit message 格式。虽然仍存在人为跳过审核的可行性，但我个人认为不必在线上再做一层审核，规范如果不能达成共识，不是所有人都认可的话，那规范也没有存在的必要了。
- ### 持续测试和集成
...
- 持续测试依赖于项目单元测试的完善，持续集成依赖于 Gitlab 与 CICD 平台的建设。这是我们在下一阶段开发基建中主要需要关心的部分。
## Unlinked References
-
-
-
